...
for FILE in $BASEDIR/riscv/*.riscv; do
  ...
  if [[ "$NAME" == "$PROGRAM"* ]]; then
    ...
    ${RISCV}nm -A $BASEDIR/sim/verilator/output/$NAME.riscv | grep -sw 'tohost' | sed -e 's/.*:\(.*\) D.*/\1/' > $BASEDIR/sim/verilator/output/$NAME.host
    ${RISCV}objcopy -O binary $BASEDIR/sim/verilator/output/$NAME.riscv $BASEDIR/sim/verilator/output/$NAME.bin
