package buffer_wires;
  timeunit 1ns; timeprecision 1ps;

  import configure::*;

  localparam BDEPTH = $clog2(BUFFER_DEPTH);

  typedef struct packed {
    logic [0:0]        wen0;
    logic [0:0]        wen1;
    logic [0:0]        wen2;
    logic [0:0]        wen3;
    logic [BDEPTH-1:0] waddr0;
    logic [BDEPTH-1:0] waddr1;
    logic [BDEPTH-1:0] waddr2;
    logic [BDEPTH-1:0] waddr3;
    logic [BDEPTH-1:0] raddr0;
    logic [BDEPTH-1:0] raddr1;
    logic [BDEPTH-1:0] raddr2;
    logic [BDEPTH-1:0] raddr3;
    logic [15:0]       wdata0;
    logic [15:0]       wdata1;
    logic [15:0]       wdata2;
    logic [15:0]       wdata3;
  } buffer_reg_in_type;

  typedef struct packed {
    logic [15:0] rdata0;
    logic [15:0] rdata1;
    logic [15:0] rdata2;
    logic [15:0] rdata3;
  } buffer_reg_out_type;

endpackage

import configure::*;
import constants::*;
import wires::*;
import buffer_wires::*;

module buffer_reg (
  input  logic               clock,
  input  buffer_reg_in_type  buffer_reg_in,
  output buffer_reg_out_type buffer_reg_out
);
  timeunit 1ns; timeprecision 1ps;

  logic [15:0] buffer_reg_array0[0:BUFFER_DEPTH-1] = '{default: '0};
  logic [15:0] buffer_reg_array1[0:BUFFER_DEPTH-1] = '{default: '0};
  logic [15:0] buffer_reg_array2[0:BUFFER_DEPTH-1] = '{default: '0};
  logic [15:0] buffer_reg_array3[0:BUFFER_DEPTH-1] = '{default: '0};

  always_ff @(posedge clock) begin
    if (buffer_reg_in.wen0 == 1) begin
      buffer_reg_array0[buffer_reg_in.waddr0] <= buffer_reg_in.wdata0;
    end
  end

  always_ff @(posedge clock) begin
    if (buffer_reg_in.wen1 == 1) begin
      buffer_reg_array1[buffer_reg_in.waddr1] <= buffer_reg_in.wdata1;
    end
  end

  always_ff @(posedge clock) begin
    if (buffer_reg_in.wen2 == 1) begin
      buffer_reg_array2[buffer_reg_in.waddr2] <= buffer_reg_in.wdata2;
    end
  end

  always_ff @(posedge clock) begin
    if (buffer_reg_in.wen3 == 1) begin
      buffer_reg_array3[buffer_reg_in.waddr3] <= buffer_reg_in.wdata3;
    end
  end

  always_comb begin
    buffer_reg_out.rdata0 = (buffer_reg_in.wen0 == 1 && buffer_reg_in.raddr0 == buffer_reg_in.waddr0
        ) ? buffer_reg_in.wdata0 : buffer_reg_array0[buffer_reg_in.raddr0];
  end

  always_comb begin
    buffer_reg_out.rdata1 = (buffer_reg_in.wen1 == 1 && buffer_reg_in.raddr1 == buffer_reg_in.waddr1
        ) ? buffer_reg_in.wdata1 : buffer_reg_array1[buffer_reg_in.raddr1];
  end

  always_comb begin
    buffer_reg_out.rdata2 = (buffer_reg_in.wen2 == 1 && buffer_reg_in.raddr2 == buffer_reg_in.waddr2
        ) ? buffer_reg_in.wdata2 : buffer_reg_array2[buffer_reg_in.raddr2];
  end

  always_comb begin
    buffer_reg_out.rdata3 = (buffer_reg_in.wen3 == 1 && buffer_reg_in.raddr3 == buffer_reg_in.waddr3
        ) ? buffer_reg_in.wdata3 : buffer_reg_array3[buffer_reg_in.raddr3];
  end

endmodule

module buffer_ctrl (
  input  logic               reset,
  input  logic               clock,
  input  buffer_in_type      buffer_in,
  output buffer_out_type     buffer_out,
  input  buffer_reg_out_type buffer_reg_out,
  output buffer_reg_in_type  buffer_reg_in
);
  timeunit 1ns; timeprecision 1ps;

  localparam W = BDEPTH + 2;
  localparam TOTAL = 4 * (BUFFER_DEPTH - 2);

  typedef struct packed {
    logic [15:0]       wdata0;
    logic [15:0]       wdata1;
    logic [15:0]       wdata2;
    logic [15:0]       wdata3;
    logic [15:0]       rdata0;
    logic [15:0]       rdata1;
    logic [15:0]       rdata2;
    logic [15:0]       rdata3;
    logic [0:0]        comp0;
    logic [0:0]        comp1;
    logic [0:0]        comp2;
    logic [W-1:0]      wid;
    logic [W-1:0]      rid;
    logic [31:0]       pc_base;
    logic [W-1:0]      diff;
    logic [W-1:0]      count;
    logic [W-1:0]      align;
    logic [31:0]       pc0;
    logic [31:0]       pc1;
    logic [31:0]       instr0;
    logic [31:0]       instr1;
    logic [0:0]        ready0;
    logic [0:0]        ready1;
    logic [0:0]        wen;
    logic [0:0]        clear;
    logic [0:0]        stall;
    logic [1:0]        rid_bank;
    logic [BDEPTH-1:0] rid_row;
    logic [BDEPTH-1:0] rid_row_p1;
    logic [BDEPTH-1:0] wid_row;
  } reg_type;

  parameter reg_type init_reg = '{
      wdata0 : 0,
      wdata1 : 0,
      wdata2 : 0,
      wdata3 : 0,
      rdata0 : 0,
      rdata1 : 0,
      rdata2 : 0,
      rdata3 : 0,
      comp0 : 0,
      comp1 : 0,
      comp2 : 0,
      wid : 0,
      rid : 0,
      pc_base : 0,
      diff : 0,
      count : 0,
      align : 0,
      pc0 : 0,
      pc1 : 0,
      instr0 : 0,
      instr1 : 0,
      ready0 : 0,
      ready1 : 0,
      wen : 0,
      clear : 0,
      stall : 0,
      rid_bank : 0,
      rid_row : 0,
      rid_row_p1 : 0,
      wid_row : 0
  };

  reg_type r, rin, v;

  always_comb begin

    v = r;

    if (buffer_in.clear == 1) begin
      v.wid   = 0;
      v.rid   = 0;
      v.count = 0;
      v.clear = 1;
    end

    if (r.clear == 1 && buffer_in.clear == 0 && buffer_in.ready == 1) begin
      v.rid     = {{W - 1{1'b0}}, buffer_in.pc0[1]};
      v.align   = {{W - 1{1'b0}}, buffer_in.pc0[1]};
      v.pc_base = {buffer_in.pc0[31:1], 1'b0};
      v.clear   = 0;
    end

    v.wen = (~buffer_in.clear) & (~r.stall) & buffer_in.ready;

    v.wid_row = v.wid[W-1:2];

    v.wdata0 = buffer_in.rdata[15:0];
    v.wdata1 = buffer_in.rdata[31:16];
    v.wdata2 = buffer_in.rdata[47:32];
    v.wdata3 = buffer_in.rdata[63:48];

    buffer_reg_in.wen0 = v.wen;
    buffer_reg_in.wen1 = v.wen;
    buffer_reg_in.wen2 = v.wen;
    buffer_reg_in.wen3 = v.wen;

    buffer_reg_in.waddr0 = v.wid_row;
    buffer_reg_in.waddr1 = v.wid_row;
    buffer_reg_in.waddr2 = v.wid_row;
    buffer_reg_in.waddr3 = v.wid_row;

    buffer_reg_in.wdata0 = v.wdata0;
    buffer_reg_in.wdata1 = v.wdata1;
    buffer_reg_in.wdata2 = v.wdata2;
    buffer_reg_in.wdata3 = v.wdata3;

    v.rid_bank   = v.rid[1:0];
    v.rid_row    = v.rid[W-1:2];
    v.rid_row_p1 = v.rid_row + 1'b1;

    buffer_reg_in.raddr0 = (v.rid_bank > 2'd0) ? v.rid_row_p1 : v.rid_row;
    buffer_reg_in.raddr1 = (v.rid_bank > 2'd1) ? v.rid_row_p1 : v.rid_row;
    buffer_reg_in.raddr2 = (v.rid_bank > 2'd2) ? v.rid_row_p1 : v.rid_row;
    buffer_reg_in.raddr3 = v.rid_row;

    case (v.rid_bank)
      2'd0: begin
        v.rdata0 = buffer_reg_out.rdata0;
        v.rdata1 = buffer_reg_out.rdata1;
        v.rdata2 = buffer_reg_out.rdata2;
        v.rdata3 = buffer_reg_out.rdata3;
      end
      2'd1: begin
        v.rdata0 = buffer_reg_out.rdata1;
        v.rdata1 = buffer_reg_out.rdata2;
        v.rdata2 = buffer_reg_out.rdata3;
        v.rdata3 = buffer_reg_out.rdata0;
      end
      2'd2: begin
        v.rdata0 = buffer_reg_out.rdata2;
        v.rdata1 = buffer_reg_out.rdata3;
        v.rdata2 = buffer_reg_out.rdata0;
        v.rdata3 = buffer_reg_out.rdata1;
      end
      default: begin
        v.rdata0 = buffer_reg_out.rdata3;
        v.rdata1 = buffer_reg_out.rdata0;
        v.rdata2 = buffer_reg_out.rdata1;
        v.rdata3 = buffer_reg_out.rdata2;
      end
    endcase

    if (v.wen == 1) begin
      v.wid   = v.wid + W'(4);
      v.count = v.count + W'(4);
    end

    v.diff = 0;

    v.comp0 = ~(&v.rdata0[1:0]);
    v.comp1 = ~(&v.rdata1[1:0]);
    v.comp2 = ~(&v.rdata2[1:0]);

    v.pc0    = 0;
    v.pc1    = 0;
    v.instr0 = 0;
    v.instr1 = 0;
    v.ready0 = 0;
    v.ready1 = 0;

    if (v.comp0 == 1) begin
      if (v.count > v.align) begin
        v.pc0    = v.pc_base;
        v.instr0 = {16'b0, v.rdata0};
        v.ready0 = 1;
        v.diff   = W'(1);
      end
    end else begin
      if (v.count > v.align + W'(1)) begin
        v.pc0    = v.pc_base;
        v.instr0 = {v.rdata1, v.rdata0};
        v.ready0 = 1;
        v.diff   = W'(2);
      end
    end

    if (v.comp0 == 1) begin
      if (v.comp1 == 1) begin
        if (v.count > v.align + W'(1)) begin
          v.pc1    = v.pc_base + 32'd2;
          v.instr1 = {16'b0, v.rdata1};
          v.ready1 = 1;
          v.diff   = W'(2);
        end
      end else begin
        if (v.count > v.align + W'(2)) begin
          v.pc1    = v.pc_base + 32'd2;
          v.instr1 = {v.rdata2, v.rdata1};
          v.ready1 = 1;
          v.diff   = W'(3);
        end
      end
    end else begin
      if (v.comp2 == 1) begin
        if (v.count > v.align + W'(2)) begin
          v.pc1    = v.pc_base + 32'd4;
          v.instr1 = {16'b0, v.rdata2};
          v.ready1 = 1;
          v.diff   = W'(3);
        end
      end else begin
        if (v.count > v.align + W'(3)) begin
          v.pc1    = v.pc_base + 32'd4;
          v.instr1 = {v.rdata3, v.rdata2};
          v.ready1 = 1;
          v.diff   = W'(4);
        end
      end
    end

    if (buffer_in.stall == 1) begin
      v.diff   = 0;
      v.ready0 = 0;
      v.ready1 = 0;
    end

    v.count   = v.count - v.diff;
    v.rid     = v.rid + v.diff;
    v.pc_base = v.pc_base + (32'(v.diff) << 1);

    if (v.count > TOTAL) begin
      v.stall = 1;
    end else begin
      v.stall = 0;
    end

    buffer_out.pc0    = v.ready0 ? v.pc0 : 32'hFFFFFFFF;
    buffer_out.pc1    = v.ready1 ? v.pc1 : 32'hFFFFFFFF;
    buffer_out.instr0 = v.ready0 ? v.instr0 : 0;
    buffer_out.instr1 = v.ready1 ? v.instr1 : 0;
    buffer_out.ready0 = v.ready0;
    buffer_out.ready1 = v.ready1;
    buffer_out.stall  = ~v.wen;

    rin = v;

  end

  always_ff @(posedge clock) begin
    if (reset == 0) begin
      r <= init_reg;
    end else begin
      r <= rin;
    end
  end

endmodule

module buffer (
  input  logic           reset,
  input  logic           clock,
  input  buffer_in_type  buffer_in,
  output buffer_out_type buffer_out
);
  timeunit 1ns; timeprecision 1ps;

  buffer_reg_in_type  buffer_reg_in;
  buffer_reg_out_type buffer_reg_out;

  buffer_reg buffer_reg_comp (
    .clock         (clock),
    .buffer_reg_in (buffer_reg_in),
    .buffer_reg_out(buffer_reg_out)
  );

  buffer_ctrl buffer_ctrl_comp (
    .reset         (reset),
    .clock         (clock),
    .buffer_in     (buffer_in),
    .buffer_out    (buffer_out),
    .buffer_reg_in (buffer_reg_in),
    .buffer_reg_out(buffer_reg_out)
  );

endmodule
